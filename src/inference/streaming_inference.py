...
def streaming_inference_logreg(model, test_seqs, config, best_thresh, mlb, 
                                   restricted_parents=None, parents_map=None, save_path=None):
    """Perform streaming inference on test embeddings using LogRegMultilabel model."""
    ...
    for i in range(0, N_test, config["PREDICT_BATCH_SIZE"]):
        ...
        # Use LogRegMultilabel's predict method (handles CuPy internally)
        y_buffer_prob = model.predict(X_buffer, batch_size=config.get("LOGREG_PREDICT_BATCH", 5000))
        
        # Optional: Propagate predictions up the hierarchy
        if config.get("PROPAGATE_PREDICTIONS", False) and parents_map and restricted_parents:
            y_buffer_prob = propagate_batch(y_buffer_prob, restricted_parents, list(mlb.classes_), iterations=3)
        
        # Write predictions for each protein in batch
        for ridx, pid in enumerate(batch_ids):
            probs = y_buffer_prob[ridx]
            predictions = get_top_k_predictions(probs, best_thresh, config["TOP_K_PER_PROTEIN"], mlb)
            for go_id, score in predictions:
